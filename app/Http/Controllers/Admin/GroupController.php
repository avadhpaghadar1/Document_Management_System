...
class GroupController extends Controller
{
    ...
    public function display(Request $request)
    {
        try {
            ...
            return view('admin/groups/index', ['groups' => $groups, 'sort_by' => $sort_by, 'sort_order' => $sort_order, 'search' => $search]);
    ...
    public function view($id)
    {
        try {
            ...
            $group = Group::find($id);
            $users = $group->users;
            return view('admin/groups/view', ['group' => $group, 'users' => $users]);
        } catch (AuthorizationException $e) {
            return redirect()->route('dashboard')->with('error', 'You do not have permission to perform this action.');
        }
    }
    public function create()
    {
        try {
            ...
            $users = User::get();
            return view('admin/groups/create', ['users' => $users]);
        } catch (AuthorizationException $e) {
            return redirect()->route('dashboard')->with('error', 'You do not have permission to perform this action.');
        }
    }
    ...
    public function edit($id)
    {
        try {
            ...
            $users = User::get();
            $members = $group->users;
            return view('admin/groups/edit', ['group' => $group, 'users' => $users, 'members' => $members]);
        } catch (AuthorizationException $e) {
            return redirect()->route('dashboard')->with('error', 'You do not have permission to perform this action.');
        }
    }
