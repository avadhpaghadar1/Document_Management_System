        ...
        <div class="col-12">
            ...
            <div class="card">
                    ...
                    <div>
                        ...
                        <div class="mt-2 ps-5 ms-3">
                            <div class="input-with-icon">
                                <x-input type="text" name="daterange" class="text-dark" placeholder="Filter by expiry date range" readonly value="{{ request('daterange') }}"></x-input>
                            </div>
                            <form id="dateRangeForm" method="GET" action="{{ route('document') }}">
                                <input type="hidden" id="start_date" name="start_date" value="{{ $startDate ?? request('start_date') }}">
                                <input type="hidden" id="end_date" name="end_date" value="{{ $endDate ?? request('end_date') }}">
                                <input type="hidden" name="daterange" id="daterange" value="{{ request('daterange') }}" />
                        ...
                        <form method="GET" id="searchForm" class="d-flex mt-2 ps-5 ms-5">
                            ...
                            <select class="form-select ms-1" name="status" aria-label="Status">
                                @php
                ...
                <table class="table table-hover my-0">
                    <thead>
                        <tr>
                            <th><a href="{{ route('document', array_merge(request()->all(), ['sort_by'=>'id','sort_order'=>$sort_order=='asc' && $sort_by =='id'?'desc':'asc'])) }}" class="text-dark">Document Id</a></th>
                            <th class="d-none d-xl-table-cell"><a href="{{ route('document', array_merge(request()->all(), ['sort_by'=>'document_type_id','sort_order'=>$sort_order=='asc' && $sort_by =='document_type_id'?'desc':'asc'])) }}" class="text-dark">Document Type</a></th>
                            <th class="d-none d-xl-table-cell"><a href="{{ route('document', array_merge(request()->all(), ['sort_by'=>'expiry','sort_order'=>$sort_order=='asc' && $sort_by =='expiry'?'desc':'asc'])) }}" class="text-dark">Status</a></th>
                            <th><a href="{{ route('document', array_merge(request()->all(), ['sort_by'=>'expiry','sort_order'=>$sort_order=='asc' && $sort_by =='expiry'?'desc':'asc'])) }}" class="text-dark">Expiring</a></th>
                            <th class="d-none d-xl-table-cell">Days Left</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @php
                                $daysLeft = \Carbon\Carbon::parse($document->expiry)->diffInDays(now(), false);
                                $daysBadge = $daysLeft > 7 ? 'bg-success' : ($daysLeft > 0 ? 'bg-warning' : 'bg-danger');
                                $daysText = $daysLeft > 0 ? $daysLeft . ' day(s)' : '0 day(s)';
                            @endphp
                            <td class="d-none d-xl-table-cell"><span class="badge {{ $daysBadge }}">{{ $daysText }}</span></td>
                        ...
                        <tr>
                            <td>
                                @can('export_document')
                                <a href="{{route("export-document")}}"><x-button class="btn-primary d-flex justify-content-start "><i class="mt-1 me-2" data-feather="upload"></i>Export</x-button></a>
                                @endcan
                            </td>
                            <td colspan="4"></td>
